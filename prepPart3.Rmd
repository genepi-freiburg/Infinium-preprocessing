...
samples <- samples[samples %in% datsamplesfinal$Sample_ID]
datsamples <- datsamplesfinal[datsamplesfinal$Sample_ID %in% samples,]
```
...
marker.callQN =rowSums(!is.na(beta))/ncol(beta) # this line is added; if problems occur, maybe delete it and rechange all lines where marker.callQN occurs to marker.call

# sex chromosomes ------------------------------------------------------------------
women <- datsamples[datsamples$Gender==1,"Sample_ID"]
men <- datsamples[datsamples$Gender==0,"Sample_ID"]
...
beta_sex <- cbind(beta_m,beta_w) 
marker.call_sex=rowSums(!is.na(beta_sex))/ncol(beta_sex)
beta_all <- rbind(beta,beta_sex)
...
ctrlprobes.scores <- merge(ctrlprobes.scores,datsamples[,c("Sample_ID","Sample_Name")],by.x=0,by.y="Sample_ID")
write.csv(ctrlprobes.scores,file=paste0(outputdir,"/PC_controls-finalSample","_",projectname,"-",formatted.time,".csv"),row.names=F)

# sex-check
sample.callY=colSums(!is.na(beta_sex[sexchrY,]))/nrow(beta_sex[sexchrY,])
sample.callX=colSums(!is.na(beta_sex[sexchrX,]))/nrow(beta_sex[sexchrX,])
...
```{r}
# chr X+Y
betaPCA <- beta_sex[c(sexchrX,sexchrY),]
...
#plot(EV[,"PC1"],EV[,"PC2"],col=as.numeric(as.factor(paste(EV$Sample_Group)))+2, main=paste0("number of X+Y probes: ",nrow(betaPCA)),xlab=paste0("EV1 (",sprintf("%1.1f",(pcaSex$sdev^2/sum(pcaSex$sdev^2))[1]*100),"%)"),ylab=paste0("EV2 (",sprintf("%1.1f",(pcaSex$sdev^2/sum(pcaSex$sdev^2))[2]*100),"%)"))
```
...
marker.callYwomen=rowSums(!is.na(beta_sex[sexchrY,colnames(beta_sex) %in% women]))/ncol(beta_sex[sexchrY,colnames(beta_sex) %in% women])
}
```
...
# test only
#plot(EV[,"PC1"],EV[,"PC2"], main=paste0("number of autosomal probes: ",nrow(pcaAuto$x)),xlab=paste0("EV1 (",sprintf("%1.1f",(pcaAuto$sdev^2/sum(pcaAuto$sdev^2))[1]*100),"%)"),ylab=paste0("EV2 (",sprintf("%1.1f",(pcaAuto$sdev^2/sum(pcaAuto$sdev^2))[2]*100),"%)"),col=as.numeric(as.factor(paste(EV$Sample_Group,EV$Sample_Plate)))+2)
#plot(EV[,"PC3"],EV[,"PC4"], main=paste0("number of autosomal probes: ",nrow(pcaAuto$x)),xlab=paste0("EV3 (",sprintf("%1.1f",(pcaAuto$sdev^2/sum(pcaAuto$sdev^2))[3]*100),"%)"),ylab=paste0("EV4 (",sprintf("%1.1f",(pcaAuto$sdev^2/sum(pcaAuto$sdev^2))[4]*100),"%)"),col=as.numeric(as.factor(EV$Sample_Group))+2)
#legend("topleft", c("pilot 1","pilot 2"), col=c(4,3), pch = 1)
```
...
marker.call_all <- data.frame(callrate=c(marker.callQN,marker.call_sex)) # marker callrate of all samples incl. excluded ones
marker.call.all = marker.call_all
# marker.call_all <- data.frame(callrate=c(marker.call,marker.call_sex)) # marker callrate of all samples incl. excluded ones
