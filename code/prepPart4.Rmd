...
outlier_mask <- sweep(betaQN.all.3IQR, 1, probe_IQR$rangeMin3IQR, "<") | 
                sweep(betaQN.all.3IQR, 1, probe_IQR$rangeMax3IQR, ">")

# Count valid probes (non-NA values)
nprobes <- rowSums(!is.na(betaQN.all.3IQR))

# Count outliers
outlier3_IQR <- rowSums(outlier_mask, na.rm = TRUE)

# Remove outliers
betaQN.all.3IQR[outlier_mask] <- NA

# Create result dataframe
marker.outlier.all <- data.frame(
  name = rownames(betaQN.all.3IQR),
  nprobes = nprobes,
  outlier3_IQR = outlier3_IQR,
  callrate = nprobes / ncol(betaQN.all.3IQR)
)
  
```

```{r, label = IQR_Nprobes, echo=FALSE}
Nprobes = as.numeric(nrow(betaQN.all)) * as.numeric(ncol(betaQN.all))
