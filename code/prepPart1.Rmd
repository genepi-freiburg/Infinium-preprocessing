...
# this could be a way to get arraytypes 
...
for(path in scandata){ # all files in the same path have to have identical array built - they are merged later
	...
	#OLD: 	dim(filenames) <- c(1,length(filenames)) #	RGset <- read.metharray(filenames[1,], verbose=TRUE, force = TRUE) 
}

# make one filenames object to merge # Test
	...
	# we need to handle samples without idat file
	 ...
	 write.table(missing_idat, file = paste0(outputdir,"/missing_idat_files","_",projectname,"-",formatted.time,".tsv"), row.names = FALSE, col.names = TRUE, append = TRUE) 
...
for(path in scandata){ # all files in the same path have to have identical array built - they are merged later
	...
	RGset = read.metharray.exp(targets = samplesheet_local, recursive = TRUE, force = TRUE) # do we want to force? occured in EPIC(v1) idats only, not a v1-v2 problem

 	# if(annotation(RGset)["array"] == "IlluminaHumanMethylationEPICv2"){
		# explained in the IlluminaHumanMethylationEPICv2anno.20a1.hg38 package https://jokergoo.github.io/IlluminaHumanMethylationEPICv2manifest/
    	# not longer necessary with minfi update
 		#	annotation(RGset)["array"] = "IlluminaHumanMethylationEPICv2" # doesnt work for EPICv2 yet
 		#	annotation(RGset)["annotation"] = "20a1.hg38"
 		
 		annotation(RGset)["array"] = "IlluminaHumanMethylationEPIC" # switch to EPIC only to make bgcorrect.illumina and wbc.est work; the function should work exactly as it should be because the arraytype only determines the name of the column of probes, it seems. Having the right column name, the probes are EPICv2-specific not EPIC (v1) downgrade. 
 	
 	# }
	
	RGset <- bgcorrect.illumina(RGset)  # Illumina background subtraction # doesnt work on minfi 1.50.0 EPIC v2 (yet) # since the control descriptor seems to be the only difference between arrays, but is the same for EPIC and EPICv2 I state that handling the EPICv2 RGset as an EPIC in bgcorrect returns the correct result; see function details and getProbeInfo(dat, "Control")
...
# if there are several paths; function can only handle two objects

RGset <- RGset_pathwise[[1]]

if(length(scandata) > 1){
	
	for(i in 2:length(scandata)){
		RGset <- combineArrays(RGset, RGset_pathwise[[i]],
                   outType = c("IlluminaHumanMethylationEPIC"), # == arraytype # currently tries to downgrade to 450k which is not good
                  verbose = TRUE)	
	}
}
...
if(exists("extractSNPs")){ ## this works on EPICv2 !! :)
...
# rest of this chunk works fine on EPICv2
	...
	##### how to handle different array types? are these different? 
...
# if data is handled successively (not in current code version)
...
anno=read.csv(annotationfile,as.is=TRUE, skip = 7) # skip=7 is true for EPICv2 as well 
...
# get autosomal probes
 cas=anno[substr(anno$Name, 1,3)=='ch.' & !(anno$CHR %in% c(chrX_descr,chrY_descr)),]
 cgs=anno[substr(anno$Name, 1,2)=='cg'& !(anno$CHR %in% c(chrX_descr,chrY_descr)),]
 auto = c(cgs$Name, cas$Name)
 auto=as.matrix(auto)

# get chrX probes # changed to work also on EPICv2
