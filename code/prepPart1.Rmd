...
# if(length(filenames)%%2==1){filenames <- c(filenames,NA)}
...
if(WBCok%in%TRUE){
 	filenames <- unique(gsub("_Grn.idat","",gsub("_Red.idat","",dir(pattern="\\.idat$",recursive=TRUE),ignore.case=T),ignore.case=T))
 	length.of.folder = max(gregexpr2("/", filenames[1])[[1]]) # we assume the pattern is identical for all measurements; if something goes missing, make some unlist here and everything gets complicated, but it's possible
