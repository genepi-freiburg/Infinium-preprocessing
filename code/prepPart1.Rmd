...
multiply_rows_V1<- function (ls, anno_v2, cgs) {
  indx<-which(arraytype =="IlluminaHumanMethylationEPIC")
  dfv1<-ls[[indx]];  dim(dfv1) # 723722     11  
  matching_rows <- dfv1[rownames(dfv1) %in% cgs, , drop = FALSE]; dim(matching_rows) # 2714   11
  cg_v2_matches <- anno_v2$IlmnID_EPICv2[anno_v2$Name %in% rownames(matching_rows)]; length(cg_v2_matches) #6246
  
  # Create matrix: repeat row according number of coincidente in cg_v2_matches
  times <- sapply(rownames(matching_rows), function(x) sum(anno_v2$Name == x))
  expanded_rows <- matching_rows[rep(1:nrow(matching_rows), times = times), , drop = FALSE]; dim(expanded_rows) # 6246   11
  #Take into account rownames
  cg_v2_matches_ordered <- unlist(sapply(unique(rownames(expanded_rows)), function(x) {
    anno_v2$IlmnID_EPICv2[anno_v2$Name == x]
  }))
  rownames(expanded_rows) <- cg_v2_matches_ordered
  #Merge data
  dfv1 <- rbind(dfv1, expanded_rows)
  ls[[indx]]<-dfv1
...
combine_data <- function(df_list) {
  df_list <- lapply(df_list, remove_pattern)
  combined_df <- Reduce(function(x, y) merge(x, y, by = "row.names", all = FALSE), df_list)
...
anno=read.csv(annotationfile,as.is=TRUE, skip = 7); dim(anno)#1085441      13

#--------------------Potential problematic problems: ---------------------------------------------
...
name_counts <- table(anno[anno$EPIC_version=="v2" & anno$Type=="Probe",]$Name)
cgs_ta_name <- names(name_counts[name_counts >= 2]); length(cgs_ta_name) #5225
anno_ta_v2<-anno[anno$Name%in%cgs_ta_name & anno$EPIC_version=="v2",]; dim(anno_ta_v2) #11622  13
#length(unique(sub("_.*", "", cgs_ta_v2))) # 5225  #Perfect. Only problematics

#Multiply problematic probes(EPICv2) in EPICv1
TypeII.Red_pathwise<-multiply_rows_V1(TypeII.Red_pathwise, anno_ta_v2, cgs_ta_name)
TypeII.Green_pathwise<-multiply_rows_V1(TypeII.Green_pathwise, anno_ta_v2, cgs_ta_name)
TypeI.Red.M_pathwise<-multiply_rows_V1(TypeI.Red.M_pathwise, anno_ta_v2, cgs_ta_name)
TypeI.Red.U_pathwise<-multiply_rows_V1(TypeI.Red.U_pathwise, anno_ta_v2, cgs_ta_name)
TypeI.Green.M_pathwise<-multiply_rows_V1(TypeI.Green.M_pathwise, anno_ta_v2, cgs_ta_name)
TypeI.Green.U_pathwise<-multiply_rows_V1(TypeI.Green.U_pathwise, anno_ta_v2, cgs_ta_name)

#Combine data		
TypeII.Red.All <-combine_data(TypeII.Red_pathwise) #612824
TypeII.Green.All <- combine_data(TypeII.Green_pathwise) #612824
TypeI.Green.M.All <- combine_data(TypeI.Green.M_pathwise) #38706    
TypeI.Green.U.All <- combine_data(TypeI.Green.U_pathwise) #38706
TypeI.Red.M.All <- combine_data(TypeI.Red.M_pathwise) #70203    
TypeI.Red.U.All <- combine_data(TypeI.Red.U_pathwise) #70203 

ctrl.all <- do.call("rbind",ctrl_pathwise) # need to transpose? 

## added for lowlevel QC by AT 
ctrl.complete.Red.all <- combine_data(ctrl.complete.Red_pathwise) #633
ctrl.complete.Green.all <- combine_data(ctrl.complete.Green_pathwise) # added for lowlevel QC  633
    
dp.all <- combine_data(dp_pathwise) #721745
