...
treatment_ta_problematic<- function (ls, anno_v2, cgs, arraytype) {
  ...
  #Save rownames vector for next steps
  dfv1_exp<-rownames(dfv1_exp)
  dfv1_match<-rownames(dfv1_match)
  rm(times, cgs_ordered)
  
  #Work with EPIC V2 --> Extract specific probes
  indx2<-which(arraytype =="IlluminaHumanMethylationEPICv2")
  dfv2<-ls[[indx2]]
  #Subset data
  dfv2<-as.data.frame(dfv2[rownames(dfv2) %in% dfv1_exp,])
  
  #Subset dfv1 according to the probes present in dfv2
  cgs_subset<-c(rownames(dfv2), 
                dfv1_match[dfv1_match%in% unique(sub("_.*", "", rownames(dfv2)))])
...
harmonize_snps <- function(ls, arraytype) {
  ...
  for (i in c(indx1,indx2)) {
  ls[[i]]<-ls[[i]][rownames(ls[[i]]) %in% snps,]
  ls[[i]] <- ls[[i]][match(snps, rownames(ls[[i]])), , drop = FALSE]
  }
  return(ls)
}
```
...
for(path in scandata){ # all files in the same path have to have identical array built - they are merged later
	...
	RGset = read.metharray.exp(targets = samplesheet_local, recursive = TRUE, verbose= TRUE, force = TRUE) # need force in EPIC(v1) idats only, not a v1-v2 problem
...
if(exists("extractSNPs")){ 
  ...
  snp.extraction<-do.call("cbind", snps_pathwise)
  	if ("IlluminaHumanMethylationEPICv2" %in% values) {
	  	  rownames(snp.extraction)<-sub("_.*", "", rownames(snp.extraction))
	  	  snp.extraction<-snp.extraction[!duplicated(rownames(snp.extraction)),]
	  }
  } 
} 
...
for(path in scandata){ # all files in the same path have to have identical array built - they are merged later
	...
	print(paste0("Complete for: ", path))
}

print("Get control probe information and P detection")
for(path in scandata){ # all files in the same path have to have identical array built - they are merged later
...
if (length(values) > 1) {
  if ("IlluminaHumanMethylationEPIC" %in% values &&
      "IlluminaHumanMethylationEPICv2" %in% values) {
    #Used fixed pipeline
print("Merging IlluminaHumanMethylationEPIC and IlluminaHumanMethylationEPICv2 arrays")
...
rm(name_counts,anno_ta_v1, v2_sp, anno_c, anno_c1, anno_c2, anno_merge, unm_dt)
...
```{r, echo=FALSE}
for (i in 1:nrow(d2r_directory)) {
  cat("Arraytype:\n")
  cat(d2r_directory$arraytype[i], "\n")
  cat(d2r_directory$directory[i], "\n\n")
}
```
...
if (length(values) > 1) {
 ...
 sexchrY=as.matrix(sexchrY)
  ...
  if ("IlluminaHumanMethylationEPICv2" %in% values) {
  anno=anno[,c('IlmnID','Color_Channel', 'CHR', 'MAPINFO', 'Name')]
  anno$CHR <- gsub("chr", "", anno$CHR)
  anno<-anno[anno$CHR %in% c(1:22, "X", "Y", "M"),]
  
# get autosomal probes
 cas=anno[substr(anno$IlmnID, 1,3)=='ch.' & !(anno$CHR %in% c('X','Y')),]
 cgs=anno[substr(anno$IlmnID, 1,2)=='cg'& !(anno$CHR %in% c('X','Y')),]
 auto = c(cgs$IlmnID, cas$IlmnID)
 auto=as.matrix(auto)

# get chrX probes
 xcas=anno[substr(anno$IlmnID, 1,3)=='ch.' & (anno$CHR %in% c('X')),]
 xcgs=anno[substr(anno$IlmnID, 1,2)=='cg'& (anno$CHR %in% c('X')),]
 sexchrX = c(xcgs$IlmnID, xcas$IlmnID)
 sexchrX=as.matrix(sexchrX)

 # get chrY probes
 ycas=anno[substr(anno$IlmnID, 1,3)=='ch.' & (anno$CHR %in% c('Y')),]
 ycgs=anno[substr(anno$IlmnID, 1,2)=='cg'& (anno$CHR %in% c('Y')),]
 sexchrY = c(ycgs$IlmnID, ycas$IlmnID)
 sexchrY=as.matrix(sexchrY)  
...
```{r, label='detection_p-values', echo=FALSE} 
dpvec <- as.vector(t(dp.all))
# dpvec <- dp.all
...
```{r, label='detection_p-values_zero', echo=FALSE}
...
```{r, echo=FALSE}
knitr::include_graphics("dpvec_histogram.pdf")
```
...
```{r, label = 'samplecall.markercall', echo=FALSE}
...
```{r, label='beta-densities', echo=FALSE}
...
```{r, echo=FALSE}
knitr::include_graphics("beta_densities_plot.pdf")
```
...
```{r, label = 'sample_call', echo=FALSE}
...
```{r, echo=FALSE}
knitr::include_graphics("sample_call.pdf")
```

```{r, label = sample_call_zoom, echo=FALSE}
# plot sample call rates
pdf("sample_call_zoom.pdf")
hist(sample.call,breaks=100, main = "sample call rates zoomed axis", xlim = c(0.85,1), ylim = c(0,30), sub = "zoomed x- and y-axis is fixed to 0.85 to 1 and 0 to 30, respectively")
dev.off()
graphics.off()
gc()
```
...
```{r, echo=FALSE}
knitr::include_graphics("sample_call_zoom.pdf")
```

```{r, label = sample_call_zoomy, echo=FALSE}
# plot sample call rates
...
```{r, label = marker_call, echo=FALSE}
#hist(marker.call,breaks=100, main = "marker call rates")
```


```{r, label = marker_call_zoom, echo=FALSE}
pdf("marker_call_zoom.pdf")
hist(marker.call,breaks=100, main = "marker call rates zoomed y-axis", ylim = c(0,500), sub = "zoomed y axis is fixed to 0 to 500")
dev.off()
graphics.off()
gc()
rm(dpvec, detectionP.local, estimateWBCs_EPICv2, treatment_ta_problematic, remove_pattern, harmonize_snps, combine_data, calculate_marker_call, calculate_sample_call, RGset_pathwise, dp_pathwise, TypeII.Green_pathwise, TypeII.Red_pathwise, TypeI.Red.M_pathwise, TypeI.Red.U_pathwise, TypeI.Green.M_pathwise, TypeI.Green.U_pathwise, ctrl_pathwise, anno, cgs,TypeII.Name, cgs_shared, cgs_epicv2, cgs_epicv1, xcgs, TypeI.Red.Name, TypeI.Green.Name, ctrl, samplesheet_combined, category, anno_ta_v2,filenames_pathwise)
```  

```{r, echo=FALSE}
knitr::include_graphics("marker_call_zoom.pdf")
```
