...
for(path in scandata){
	...
	filenames_tmp <- unique(gsub("_Grn.idat","",gsub("_Red.idat","",files_here,ignore.case=T),ignore.case=T))
	
	filenames <- c(filenames, filenames_tmp)
	rm(files_here,filenames_tmp)
}

sample_names = sapply(filenames, function(s){sub(".+/","",s)})
...
## this returns a list, we need an index
	 filenames_idx <-  as.integer(filenames_idx_list)
	 
#		# we need to handle samples without idat file
	missing_idat <- names(filenames_idx_list[is.na(filenames_idx)])
	write.table(missing_idat, file = "missing_idat_files.tsv", row.names = FALSE, col.names = TRUE)
...
# for(i in 1:5){ # test version
	...
	RGset <- read.metharray(filenames[i,], verbose=TRUE, force = TRUE) # formerly: read.450k()
