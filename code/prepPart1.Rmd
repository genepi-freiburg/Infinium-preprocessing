...
```{r, label='read-in', message = FALSE, echo = FALSE, warning = FALSE}
...
```{r, label='read-in.Part2', message = FALSE, echo = FALSE, warning = FALSE}
...
for(path in scandata){ # all files in the same path have to have identical array built - they are merged later
	...
	files_here <- dir(pattern="\\.idat$",recursive=TRUE, full.names=TRUE, path = getwd()) # this path handling ensures correct number of / in path name
	...
	# we need to handle samples without idat file
	 missing_idat <- c(missing_idat, names(filenames_idx_list[is.na(filenames_idx)]))
	 write.table(missing_idat, file = "missing_idat_files.tsv", row.names = FALSE, col.names = TRUE) # need overwrite = TRUE?
	
	# need unique NA-free index 
	 filenames_idx <- filenames_idx[!is.na(filenames_idx)]

# 	 filenames_final_idx <- as.integer(sapply(datsamplesfinal$Sentrix_Code, function(x){grep(x,filenames)}))
# 	 filenames_final_idx <- filenames_final_idx[!is.na(filenames_final_idx)]
...
# 	dim(filenames) <- c(1,length(filenames))
	...
	### TODO RGset = read.metharray.exp(samplesheet,...)

	if-clause if(arraytype == "IlluminaHumanMethylationEPICv2")
...
#	RGset <- read.metharray(filenames[1,], verbose=TRUE, force = TRUE) 
	RGset <- read.metharray(filenames, verbose=TRUE, force = TRUE) 
...
if(length(scandata) > 1){
	...
	for(i in 2:length(scandata)){
		RGset <- combineArrays(RGset, RGset_pathwise[[i]],
                  outType = c("IlluminaHumanMethylationEPIC"), # == arraytype
                  verbose = TRUE)	
	}
}
...
######## WBC and SNP extraction ####################
	...
	# add: make WBC estimation with samples from *final* samplesfile
		...
		#  	RGset <- bgcorrect.illumina(RGset)  # Illumina background subtraction 
 		...
 		if(exists("est.wbc.minfi")){ # in code as is this is needless since i is only 1 and all files are read simultaneously
...
########### PROBE INFO & DATA SPLIT ###############################################
	...
	#### TODO merge all data of different EPIC versions later ### 
...
rm(dp, detppre, controlExtRed, controlExtGreen, extendedTypeGreen, extendedTypeRed, filenames_idx_list, filenames_idx, i, idatdir, rawtypepre)
