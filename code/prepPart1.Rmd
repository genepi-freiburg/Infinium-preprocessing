...
treatment_ta_problematic<- function (ls, anno_v2, cgs, arraytype) {
  ...
  dfv1<-ls[[indx1]];  dim(dfv1) # 723722     11  
  dfv1_match <- dfv1[rownames(dfv1) %in% cgs, , drop = FALSE]; dim(dfv1_match) # 2714   11

  # Create matrix: repeat row according number of coincidente in cg_v2_matches
  times <- sapply(rownames(dfv1_match), function(x) sum(anno_v2$Name == x))
  dfv1_exp <- dfv1_match[rep(1:nrow(dfv1_match), times = times), , drop = FALSE]; dim(dfv1_exp) # 6246   11
  #Rownames
  ...
  dfv1 <- rbind(dfv1_match, dfv1_exp); dim(dfv1) #8960
  
  #Work with EPIC V2 --> Extract specific probes
  indx2<-which(arraytype =="IlluminaHumanMethylationEPICv2")
  dfv2<-ls[[indx2]];  dim(dfv2) # 808719     4 
  #Subset data
  dfv2<-as.data.frame(dfv2[rownames(dfv2) %in% rownames(dfv1_exp),]); dim(dfv2) #6176  4
  
  #Subset dfv1 according to the probes present in dfv2
  cgs_subset<-c(rownames(dfv2), 
                rownames(dfv1_match)[rownames(dfv1_match)%in% unique(sub("_.*", "", rownames(dfv2)))]); length(cgs_subset) #8890
  
  dfv1<-as.data.frame(dfv1[rownames(dfv1) %in% cgs_subset,]); dim(dfv1) # 8890   11
  
  #Merge data in a single df
  dfv1$ID <- rownames(dfv1)
  dfv2$ID <- rownames(dfv2)
  df_ta<-merge(dfv1, dfv2, by="ID", all=TRUE); dim(df_ta)
...
combine_data <- function(df_list, anno_v2, arraytype, cgs_ta, cgs_shared, cgs_epicv1, cgs_epicv2) {
  ...
  df_ta<-treatment_ta_problematic(ls=df_list, anno_v2=anno_v2, cgs=cgs_ta, arraytype=arraytype); dim(df_ta) #8890
  
  #STEP 2: Combine same IDs
  df_shared <- lapply(df_list, remove_pattern) #[1] 723722     11   [1] 808719      4
  df_shared <- lapply(df_shared, function(df) {
    df[rownames(df) %in% cgs_shared, ]
  }) #Dim:610110     11 and  610110      4
  df_comb_shared <- Reduce(function(x, y) merge(x, y, by = "row.names", all = FALSE), df_shared); dim(df_comb_shared) #610110     16
  ...
  df <- df[ , all_cols, drop = FALSE] # Mantener el orden de todas las columnas
  return(df)
  }
  ...
  df_all <- rbind(df_ta, df_comb_shared, df_comb_specific)
  
  return(df_all)
}
```

```{r, label = call rate, echo = FALSE}
calculate_marker_call<- function  (cgs_ta, cgs_shared, cgs_epicv1, cgs_epicv2, 
                                   anno ,beta_df, sample.name_pathwise, arraytype) {
  #v1
  cgs_v1<- unique(c(cgs_ta, cgs_epicv1));  length(cgs_v1) #148051
  #v2
  cgs_v2<-cgs_epicv2
  # v1 and 2
  cgs_epicv12<-unique(c(cgs_ta,cgs_shared)); length(cgs_epicv12) #721447
  cgs_epicv12_suf<-unique(anno$IlmnID_EPICv2[anno$Name %in% cgs_epicv12]); length(cgs_epicv12_suf) #726103
  cgs_epicv12<- cgs_epicv12[!cgs_epicv12 %in% cgs_v1]; length(cgs_epicv12) #717868
  cgs_epicv12<-unique(c(cgs_epicv12, cgs_epicv12_suf)); length(cgs_epicv12) #1443971
  cgs_epicv12<-cgs_epicv12[!cgs_epicv12 %in%cgs_v2]; length(cgs_epicv12) #1443950
  
  #Create subdata
  indx1<-which(arraytype =="IlluminaHumanMethylationEPIC")
  indx2<-which(arraytype =="IlluminaHumanMethylationEPICv2")
  
  df_v1<- beta_df [rownames(beta_df) %in% cgs_v1, sample.name_pathwise[[indx1]]]; dim(df_v1) #144616
  df_v2<- beta_df [rownames(beta_df) %in% cgs_v2, sample.name_pathwise[[indx2]]]; dim(df_v2) #194808
  df_v12<- beta_df [rownames(beta_df) %in% cgs_epicv12, unlist(sample.name_pathwise)]; dim(df_v12) #709771
...
calculate_sample_call<- function  (cgs_ta, cgs_shared, cg_shared, cgs_epicv1, cgs_epicv2, 
  ...
  df_v1<- beta_df [rownames(beta_df) %in% c(cgs_v1,cgs_epicv12), 
                   sample.name_pathwise[[indx1]]]; dim(df_v1) #854387
  df_v2<- beta_df [rownames(beta_df) %in% c(cgs_v2,cgs_epicv12), 
                   sample.name_pathwise[[indx2]]]; dim(df_v2) #904579
...
# 	 	RGset_1 <- RGset_pathwise[[path]] 
# 	 	RGset_2 <- RGset_pathwise[[path]] 
# 	 	snp.1<- getSnpBeta(RGset_1); dim(snp.1) #59 11
# 	 	snp.2<- getSnpBeta(RGset_2); dim(snp.2) #65 4
# 	 	snp.2<- getSnpBeta(RGset_2); dim(snp.2) #65 4
# 	 	v1<-rownames(snp.1)
# 	 	v2<-sub("_.*", "", rownames(snp.2))
# 	 	
...
# 	 	vp<-v2[!v2%in%v1]
# > vp
# [1] "rs5931272"  "rs3818562"  "rs1414097"  "rs10774834" "rs5936512"
	 	
	  	snps_pathwise[[path]] <- getSnpBeta(RGset) #dim: V1=59
...
for(path in scandata){ # all files in the same path have to have identical array built - they are merged later
	RGset <- RGset_pathwise[[path]] 
	# Type II probes
	TypeII.Name <- getProbeInfo(RGset, type = "II")$Name #lenght: V1; V2--> ;808719
	TypeII.Green <- getGreen(RGset)[getProbeInfo(RGset, type = "II")$AddressA,] 
	#dim: V1; V2--> ;808719      4
	TypeII.Red <- getRed(RGset)[getProbeInfo(RGset, type = "II")$AddressA,] #dim: V1; V2--> ;808719      4
	...
	TypeI.Green.Name <- getProbeInfo(RGset, type = "I-Green")$Name #lenght: V1; V2--> ;45226
	...
	TypeII.Red_pathwise[[path]] <- TypeII.Red # dim V1 vs V2 --> 723722;11  vs  808719;4
	TypeII.Green_pathwise[[path]] <- TypeII.Green # dim V1 vs V2 --> 723722;11  vs  808719;4
	TypeI.Red.M_pathwise[[path]] <- TypeI.Red.M # dim V1 vs V2 --> 92198;11  vs  83045;4
	TypeI.Red.U_pathwise[[path]] <- TypeI.Red.U # dim V1 vs V2 --> 92198;11  vs  83045;4
	TypeI.Green.M_pathwise[[path]] <- TypeI.Green.M # dim V1 vs V2 --> 49939;11  vs  45226;4
	TypeI.Green.U_pathwise[[path]] <- TypeI.Green.U # dim V1 vs V2 --> 49939;11  vs  45226;4
}
...
anno=read.csv(annotationfile,as.is=TRUE, skip = 7); dim(anno)#1085683      10
...
cgs_ta <- names(name_counts[name_counts >= 2]); length(cgs_ta) #5169  Perfect

anno_ta_v2<-anno[anno$Name%in%cgs_ta & anno$EPIC_version=="v2" &
                   !anno$CHR == "0",]; dim(anno_ta_v2) #11423  13
anno_ta_v1<-anno[anno$EPIC_version=="v1" & anno$Name%in%cgs_ta  ,]; dim(anno_ta_v1) #3578  13
v2_sp<-unique(anno_ta_v2$Name[!anno_ta_v2$Name %in% anno_ta_v1$Name]); length(v2_sp)  #1590 son epic v2 specific
cgs_ta <- cgs_ta[!cgs_ta%in%v2_sp]; length(cgs_ta) #3579  


#Probes EPIC-version-specific:
cgs_epicv1<- as.vector(na.omit(anno$IlmnID_EPICv1[anno$Type=="Probe" & anno$EPIC_version=="v1"])); length(cgs_epicv1) #148050

cgs_epicv2<- as.vector(na.omit(anno$IlmnID_EPICv2[anno$Type=="Probe" & anno$EPIC_version=="v2" &
                                           ! anno$Name %in% cgs_ta])); length(cgs_epicv2) #210812

#Problematics
  anno_c<-anno[anno$Name %in% cgs_ta & anno$EPIC_version!="v1_v2",]; dim(anno_c)  #11895    10
  anno_c1<-anno_c[anno_c$EPIC_version=="v1" , ];  dim(anno_c1) #3578     10
  anno_c2<-anno_c[anno_c$EPIC_version=="v2",  ]; dim(anno_c2) #8317     10
  
  anno_merge <- merge(anno_c1, anno_c2, 
                  by = c("Name", "Infinium_Design_Type", "Color_Channel", "CHR"),
                  all = TRUE); dim(anno_merge)  #8346     16
unm_dt <- anno_merge[is.na(anno_merge$IlmnID_EPICv1.x) | is.na(anno_merge$AddressA_ID_EPICv1.x), ]; dim(unm_dt) #104
unm_dt<-unm_dt [unm_dt$CHR!= "0",]; dim(unm_dt) #21  here the problematics
cgs_epicv2<-c(cgs_epicv2, unm_dt$IlmnID_EPICv2.y); length(cgs_epicv2) #210833

#Probes shared between EPIC versions no problematics
cgs_shared<-as.vector(na.omit(anno$Name[anno$Type=="Probe" & anno$EPIC_version=="v1_v2"])); length(cgs_shared) #717868

#Combine data		
TypeII.Red.All <-as.matrix(combine_data(df_list=TypeII.Red_pathwise, anno_v2=anno_ta_v2, arraytype=arraytype,
                              cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2))
TypeII.Green.All <- as.matrix(combine_data(df_list=TypeII.Green_pathwise, anno_v2=anno_ta_v2, arraytype=arraytype,
                              cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2))
TypeI.Green.M.All <- as.matrix(combine_data(df_list=TypeI.Green.M_pathwise, anno_v2=anno_ta_v2, arraytype=arraytype,
                              cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2))   
TypeI.Green.U.All <- as.matrix(combine_data(df_list=TypeI.Green.U_pathwise, anno_v2=anno_ta_v2, arraytype=arraytype,
                              cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2))
TypeI.Red.M.All <- as.matrix(combine_data(df_list=TypeI.Red.M_pathwise, anno_v2=anno_ta_v2, arraytype=arraytype,
                              cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2))  
TypeI.Red.U.All <- as.matrix(combine_data(df_list=TypeI.Red.U_pathwise, anno_v2=anno_ta_v2, arraytype=arraytype,
                              cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2))
...
dp.all <- as.matrix(combine_data(df_list=dp_pathwise, anno_v2=anno_ta_v2, arraytype=arraytype,
                              cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2)); dim(dp.all) #1088435   #
...
#  [5] "cg16781484_TC111" "cg23509869_TC111" "cg23729101_TC111" "cg01341123_TC111"
#  [9] "cg25946389_TC111" "cg26201213_BC111" "cg19709977_TC111" "cg19968202_TC111"
# [13] "cg20510225_BC111" "cg10322254_BC111" "cg10387956_BO111" "cg13699845_TC111"
# [17] "cg00101235_TO111" "cg02022136_BC211" "cg02802904_TC211" "cg14194875_BC211"
# [21] "cg23998405_BC211"
dp.all<-dp.all[rownames(dp.all) %in% x,]; dim(dp.all) #1088435      15
...
```{r, label = 'annotation'}
 ...
 anno<-anno[anno$CHR %in% c(1:22, "X", "Y"),]
...
sample.call<-calculate_sample_call(cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2, 
                                   anno=anno, beta_df=beta.raw, sample.name_pathwise=sample.name_pathwise, arraytype=arraytype)
marker.call<-calculate_marker_call(cgs_ta=cgs_ta, cgs_shared=cgs_shared, cgs_epicv1=cgs_epicv1, cgs_epicv2=cgs_epicv2, 
                                   anno=anno, beta.df=beta.raw, sample.name_pathwise=sample.name_pathwise, arraytype=arraytype)
