...
The following plots show these principal components coloured by Sentrix_ID and Sample_Plate.
Because color coding for more than 25 groups is problematic, we split the batch variable in disjoint random groups of maximum 24 members and plot each group together with a 25th member who summarizes the other batches. For each group of 24 batches, we plot the PCs 1&2 and 3&4. 
...
make.fancy.PCA.plots = function(batchvariable){
	matched_category = datsamples[match(datsamples$Sentrix_Code, rownames(pcaSamples$x)),batchvariable] 
	if(length(unique(matched_category)) < 2){ 
	...
	# We can only plot up to 25 plates at the same time, so we plot 24 single plates and one (all-others-plate)
	number.of.blocks = (length(unique(matched_category))%/%24) + 1
	folds = createFolds(1:length(unique(matched_category)),k = number.of.blocks, list = FALSE)
	
	for( i in 1:number.of.blocks){
		plot_categories = matched_category
		plot_categories[!plot_categories%in%(unique(matched_category)[folds%in%i])] <- paste("all other",batchvariable)
