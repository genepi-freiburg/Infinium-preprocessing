...
```{r, label = QN_calculation, echo=FALSE}
...
#Paint by colours
color_map <- as.factor(EV$Pool_ID)
colors <- rainbow(length(unique(color_map)))[as.numeric(color_map)]

#pdf("PCA_control_probes_batches.pdf")
plot(EV[,"PC1"],EV[,"PC2"], col = colors,
     main=paste0("PCA - number of controls probes: ",ncol(datpca)),xlab=paste0("PC1 (",sprintf("%1.1f",(pcaControls$sdev^2/sum(pcaControls$sdev^2))[1]*100),"%)"),ylab=paste0("PC2 (",sprintf("%1.1f",(pcaControls$sdev^2/sum(pcaControls$sdev^2))[2]*100),"%)"))
legend("topright", legend = levels(color_map), col = rainbow(length(levels(color_map))), pch = 19, title = "Pol ID")
...
sexchrY <- intersect(sexchrY,rownames(betaQN.sex))
sexchrX <- intersect(sexchrX,rownames(betaQN.sex))
...
```{r, label = histogram_womensexcall2, echo=FALSE}
if(length(women) > 1){
par(mfrow=c(1,1))
...
```{r, label = pca_autosome_postQN, echo=FALSE}
...
```{r, label = plot_pca_autosome_postQN, echo=FALSE}
# EVals
par(mfrow=c(1,1))
...
```{r, warnings = FALSE, plot_transposed_pca_autosome_postQN, echo=FALSE} 
...
```{r, label = plot_samplevariance_postQN, echo=FALSE} 
...
```{r, warnings = FALSE, label = plots_batcheffects_postQN_sentrix, echo=FALSE} 
...
```{r, label = plots_batcheffects_postQN_sentrix2, echo=FALSE}
...
```{r, label = plots_batcheffects_postQN_additional, echo=FALSE}
