...
Now a PCA will be conducted on autosomal beta values after quantile normalisation. Here we investigate how well the marker can be separated by choosing different base vectors. 
...
The next PCA is also calculated on the quantile normalized beta values (omitting missing values), but now such that the first principal component explains most of the variation of the autosomal sample data. So the variation which is explained by the first pricipal component indicates how well the samples can be separated by choosing a new base vector.
We plot the samples in the span of the corresponding principal components coloured by batches (autosomal beta after QN). 
The colouring by Sentrix ID is saved in a separate pdf-file `r paste0("results-PCAauto-QN-batchwise","_",projectname,"-",formatted.time,".pdf")`  in `r outputdir`. 
If there are more than 121 different Sentrix IDs, this plot may have to be modified because the number of colors is exhausted.


```{r, warnings = FALSE} 
pca.col.plot2 <- function(pc, group,legend,col_pal){
p1 <- fviz_pca_ind(pc,axes=c(1,2),
		     col.ind = as.factor(c(1:25,32:127)[group]), # color by groups # if there are more than 121 groups, this will cause a warning or an error !!
             palette = col_pal, geom="point",
             addEllipses = TRUE, # Concentration ellipses
             ellipse.type = "confidence", legend.title = "", repel = FALSE)

p2 <- fviz_pca_ind(pc,axes=c(3,4),
		     col.ind = as.factor(c(1:25,32:127)[group]), # color by groups
             palette = col_pal, geom="point",
             addEllipses = TRUE, # Concentration ellipses
             ellipse.type = "confidence", legend.title = "", repel = FALSE)
...
# pdf(file=paste0(outputdir,"/results-PCAauto-QN-sentrixwise","_",projectname,"-",formatted.time,".pdf"),height=21,width = 21)
# par(mar=c(8.1,7.1,4.1,2.1))
 par(mar=c(8,7,4,2))
matched_category = datsamples$Sentrix_ID[match(datsamples$Sentrix_Code, rownames(pcaSamples$x))] 

tmp <- pca.col.plot2(pc = pcaSamples,
					 group=as.factor(matched_category),
					 legend=levels(as.factor(matched_category)),
					 col_pal=rainbow(n=length(unique(matched_category)))
					)
tmp[[1]]
tmp[[2]]
tmp[[3]]
...
The colouring by Sample Plate is saved in a separate pdf-file `r paste0("results-PCAauto-QN-platewise","_",projectname,"-",formatted.time,".pdf")`  in `r outputdir`.
...
matched_category = datsamples$Sample_Plate[match(datsamples$Sentrix_Code, rownames(pcaSamples$x))] 
