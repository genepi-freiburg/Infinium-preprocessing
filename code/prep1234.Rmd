...
QNonlyfilename = paste0("QNonly_data_",projectname,"_",formatted.time,".Rdata")
...
5. The beta values with Quantile Normalisation but not IQR are saved seperately as `r QNonlyfilename`:
\begin{description}
...
6. The data needed for further analysis is saved as `r analysisfilename`:
\begin{description}
	...
	\item[betaQN.all.3IQR] modified and combined \textbf{betaQN} and \textbf{betaQN.sex}: IQR outliers are filtered out
\end{description}


```{r}
if(exists("est.wbc.minfi")){
save(pcaControls, betaQN.all.3IQR, est.wbc.minfi, file=paste0(outputdir,"/",analysisfilename))
} else {
  save(pcaControls, betaQN.all.3IQR, file=paste0(outputdir,"/",analysisfilename))
}

save(betaQN, betaQN.sex, file=paste0(outputdir,"/",QNonlyfilename))
```
