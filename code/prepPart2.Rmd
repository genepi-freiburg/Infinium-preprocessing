...
# 	QN.filter.df = data.frame(sample = bufout.callrate.typewise.auto, filter = paste0("for Quantile Normalisation: callrate.typewise.autosomes <= ", minimum_callrate_QN), stringsAsFactors= F)
#  	QN.filter.df <- merge(QN.filter.df,datsamples,all.x=T,by.x="sample",by.y="Sentrix_Code")
# 	samples.filtered <- merge(samples.filtered,QN.filter.df,all=T)	
#   write.csv(samples.filtered,file = paste0(outputdir,"/samples-filtered","_",projectname,"-",formatted.time,".csv"),row.names=F)
# }
# 
# # add sex callrate filtered samples to samplesfile
