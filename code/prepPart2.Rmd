...
# parameters
sex.mismatch.callrate.threshold <- 0.99 # in case there are too many missing sex probes
...
if(min(dim(na.omit(betaPCA))) > 0){ # code crashes if all cg sites have missings
	...
	cat("Sex mismatch evaluation failed. \nAll sex-specific cg sites were excluded due to missing data in one or more samples.\n")

	sex.mismatches <- NA

	print("Count of sex chromosome cg sites:")
	print(nrow(betaPCA))

	print("Summary on overall relative missing data (all samples):")
	countna <- apply(beta.raw, 2, function(x){sum(is.na(x))})
	print(round(summary(countna), digits = 4))

	countsexna <- apply(betaPCA, 2, function(x){sum(is.na(x))})
	countna.relative <- countsexna/(dim(betaPCA)[1])
	countna.all <- as.data.frame(rbind(countsexna, countna.relative, countna))
	countna.all$subset <- c("number.of.missings.sex.specific.cpg.sites","percentage.of.missings.sex.specific.cpg.sites","number.of.missings.all.sites")

	cat("Overall and sex-specific missings were reported\n in a file named 'count-missing' \n as additional information on call-rates." )
	write.csv(countna.all,file = paste0(outputdir,"/count-missing","_",projectname,"-",formatted.time,".csv"),row.names=F)

# ##### version 2
# 
# 	countsexna <- apply(betaPCA, 2, function(x){sum(is.na(x))})
# 	countna.relative <- countna/(dim(betaPCA)[1])
# 	# exclude samples with sex cpg site callrate lower or equal than 0.99
# 	betaPCA <- betaPCA[,countna.relative > sex.mismatch.callrate.threshold]
# 
# 	# add sex callrate filtered samples to samplesfile
# 	sex.callrate.filtered.samples <- colnames(betaPCA)[countna.relative < sex.mismatch.callrate.threshold] # maybe with intersect and names() to avoid mistakes?
...
if(min(dim(na.omit(betaPCA))) > 0){
	...
	if(min(dim(na.omit(betaPCA))) > 0){
		...
		cat("Due to extremely high missingness samples with callrate <", sex.mismatch.callrate.threshold, "\n were excluded from sex mismatch analysis.\n We recommend to exclude these samples from analysis alltogether.\n")
