...
if(dim(na.omit(betaPCA))[1] > 0){ # code crashes if all cg sites have missings
...
countna <- apply(betaPCA, 2, function(x){sum(is.na(x))})
countna.relative <- countna/dim(betaPCA)[1]
print(round(summary(countna.relative), digits = 4))

countsexna <- apply(betaPCA, 2, function(x){sum(is.na(x))})
countna.all <- as.data.frame(rbind(countna, countna.relative, countsexna))
countna.all$subset <- c("number.of.missings.in.all.cpg.sites","percentage.of.missings.all.cpg.sites","number.of.missings.in.sex.specific.cpg.sites")
...
sex.mismatches <- NA
cat("Impossible due to missing data.\n")
