...
```{r, label = declare_remOutlier_function, echo = FALSE}
gc()
...
removeOutlier <- function(val1,val2,labels=NULL,xname="val1",yname="val2",title="",filterYout=1, SDrange=4 ,nullOffs=0.0001 ) {
    ...
    y.w <- y[which(x>log2(nullOffs))] # remove extreme values that are zero, i.e. for staining, remove them also from y (based on x-extremes)
     x.w <- Winsorize(x.w) # only works with DescTools
     y.w <- Winsorize(y.w)
...
```{r, label = pca_controlprobe, echo=FALSE}
...
* Then we look at the controls (if FilterOutlierQC == TRUE).
```{r, label = controlprobe_QC, echo=FALSE}
if(filterOutlierCtrlQC){

#control probe based QC
# Details are given in the ILMN HD methylation assay protocol guide (15019519)

# bisulfate conversion: a non-CpG sequence with only one C at the target site will be converted to T by BS conversion. Therefore, converted control probes should be expressed (for each red/green channel) and unconverted not.
# BS-I
...
```{r, label = controlprobeQC_bisulfite, echo=FALSE}
...
```{r, echo=FALSE}
knitr::include_graphics("BS-I C_U (red).pdf")
knitr::include_graphics("BS-I C_U (green).pdf")
knitr::include_graphics("BS-II (red vs. green).pdf")
```


We also check the Hybridisation of the amplified DNA to the array:

```{r, label = controlprobeQC_other, echo=FALSE}

if(filterOutlierCtrlQC){
# Hyb + TR
pdf("Hybridization (green).pdf")
...
pdf("Staining (red) Upper left cluster.pdf")
...
dev.off()
pdf("Staining Red (cross-check with Extension outliers).pdf")
...
pdf("Staining (green) - Upper left cluster.pdf")
...
pdf("cross-check with Extension outliers.pdf")
...
```{r, echo=FALSE}
knitr::include_graphics("Hybridization (green).pdf")
knitr::include_graphics("Specificity I (red).pdf")
knitr::include_graphics("Specificity I (green).pdf")
knitr::include_graphics("Specificity II.pdf")
knitr::include_graphics("Extensions.pdf")
knitr::include_graphics("Staining (red) Upper left cluster.pdf")
knitr::include_graphics("Staining Red (cross-check with Extension outliers).pdf")
knitr::include_graphics("Staining (green) - Upper left cluster.pdf")
knitr::include_graphics("Staining (red vs. green).pdf")
knitr::include_graphics("cross-check with Extension outliers.pdf")
```
...
```{r, label = controlprobeQC_report, echo=FALSE}
if(nrow(samples.filtered) > 0){
...
# 	print(reduced.samples.filtered[,-c(posName,posFilter)])
  ...
  pdf("beta_auto_samples_filtered.pdf")
...
```{r, echo=FALSE}
knitr::include_graphics("beta_auto_samples_filtered.pdf")
```

### Sex mismatch

Gamete methylation can be used to check sex mismatches. 
```{r, label = sexmismatch, echo=FALSE}
############## sex mismatch ##############
...
# Raw betas
sexprobes <- intersect(rownames(beta.raw.sex), c(sexchrX,sexchrY)) # chr X+Y # this is necessary fro stability if different arrays are involved and e.g. EPICv2 array is only processed on EPIC v1 sites.
...
if(min(dim(na.omit(betaPCA))) > 0){ # otherwise code would crash if all cg sites have missings
	...
	idx = which(EV$Sex_female%in%c(1)) # 1 is female
	 pdf("find female outlier.pdf")
...
```{r, echo=FALSE}
if (file.exists("find female outlier.pdf")) {
  knitr::include_graphics("find female outlier.pdf")
}
if (file.exists("find male-outlier.pdf")) {
  knitr::include_graphics("find male-outlier.pdf")
}
if (file.exists("find female-outlier_2.pdf")) {
  knitr::include_graphics("find female-outlier_2.pdf")
}
if (file.exists("find male-outlier_2.pdf")) {
  knitr::include_graphics("find male-outlier_2.pdf")
}
```
...
```{r, label = printsexmismatch, echo=FALSE}
print(sex.mismatches)
rm(sex.mismatches)
```

The plot shows the principle components of the PCA on the samples in the space of samples (a standard PCA plot).

```{r, label = pca_sexmismatch, echo=FALSE}
if(exists("EV")){
	resSexCheck <- data.frame(Sentrix_Code=EV$Sentrix_Code, Sex=EV$Sex_female, EV1=EV[,"PC1"], EV2=EV[,"PC2"])
} else {
	resSexCheck <- NULL
}
...
if(min(dim(na.omit(betaPCA))) > 0){
	...
	# plot first males, then females: find female outlier
	idx = which(EV$Sex_female%in%c(1)) # 1 is female
	pdf("PCA_find female-outlier.pdf")
	...
	if(min(dim(na.omit(betaPCA))) > 0){
		...
		# plot first males, then females: find female outlier
		idx = which(EV$Sex_female%in%c(1)) # 1 is female
		pdf("PCA_find female-outlier_2.pdf")
		plot(EV[-idx,"PC1"],EV[-idx,"PC2"],col="orange", main=paste0("number of sex probes: ",nrow(betaPCA)," (RAW NA omitted)"),sub = "check for female outlier", xlab=paste0("PC1 (",sprintf("%1.1f",(pcaSex$sdev^2/sum(pcaSex$sdev^2))[1]*100),"%)"),ylab=paste0("PC2 (",sprintf("%1.1f",(pcaSex$sdev^2/sum(pcaSex$sdev^2))[2]*100),"%)"), xlim = c(min(EV[,"PC1"]),max(EV[,"PC1"])+ 0.25*( max(EV[,"PC1"]) - min(EV[,"PC1"])  )), ylim = c(min(EV[,"PC2"]),max(EV[,"PC2"]) ))
		points(EV[idx,"PC1"],EV[idx,"PC2"],col="blue")
		legend("top", pch = c(1,1), col = c("blue","orange"), legend = c("women","men"))
		dev.off()
		...
		# then plot first females, then males: find male-outlier
		pdf("PCA_find male-outlier_2.pdf")
		plot(EV[idx,"PC1"],EV[idx,"PC2"],col="blue", main=paste0("number of sex probes: ",nrow(betaPCA)," (RAW NA omitted)"),sub = "check for male outlier", xlab=paste0("PC1 (",sprintf("%1.1f",(pcaSex$sdev^2/sum(pcaSex$sdev^2))[1]*100),"%)"),ylab=paste0("PC2 (",sprintf("%1.1f",(pcaSex$sdev^2/sum(pcaSex$sdev^2))[2]*100),"%)"), xlim = c(min(EV[,"PC1"]),max(EV[,"PC1"])+ 0.25*(max(EV[,"PC1"]) - min(EV[,"PC1"])  )), ylim = c(min(EV[,"PC2"]),max(EV[,"PC2"]) ))
		points(EV[-idx,"PC1"],EV[-idx,"PC2"],col="orange")
		legend("top", pch = c(1,1), col = c("blue","orange"), legend = c("women","men"))
...
```{r, echo=FALSE}
#mod
if (file.exists("PCA_find female-outlier.pdf")) {
  knitr::include_graphics("PCA_find female-outlier.pdf")
}
if (file.exists("PCA_find male-outlier.pdf")) {
  knitr::include_graphics("PCA_find male-outlier.pdf")
}
if (file.exists("PCA_find female-outlier_2.pdf")) {
  knitr::include_graphics("PCA_find female-outlier_2.pdf")
}
if (file.exists("PCA_find male-outlier_2.pdf")) {
    knitr::include_graphics("PCA_find male-outlier_2.pdf")
}
```
...
```{r, label = report_sexmismatch, echo=FALSE}
...
```{r, label = typewise_callrate.QNfilter_auto, echo=FALSE}
...
```{r, label = men.women, echo=FALSE}

# sex chromosomes ------------------------------------------------------------------
women <- datsamples[datsamples$Sex_female==1,"Sentrix_Code"]
men <- datsamples[datsamples$Sex_female==0,"Sentrix_Code"]
```

Of the persons from whom samples are processed, `r length(women)` are women and `r length(men)` men according to the samplesfile.   



```{r, label = QNfilters_sex, echo=FALSE}
...
Of the persons from whom samples are processed, `r length(women)` are women and `r length(men)` men have usable sex chromosome data.   
