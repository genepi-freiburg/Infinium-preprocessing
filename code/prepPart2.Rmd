...
##### version 2
	...
	betaPCA <- betaPCA[,countna.relative < (1 - sex.mismatch.callrate.threshold)] # high callrate means low percentage of missings 
...
if(min(dim(na.omit(betaPCA))) > 0){
	...
	countsexna <- apply(betaPCA, 2, function(x){sum(is.na(x))})
	countna.relative <- countsexna/dim(betaPCA)[1]
	# exclude samples with sex cpg site callrate lower or equal than 0.99 / threshold
